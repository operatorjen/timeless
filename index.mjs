export class Timeless {
  ...
  _setDigits() {
    return Math.ceil(Math.log(this.alphabet.length) / Math.log(this.base.length)) || 1
  }
  ...
  setSymbols(syms) {
    if (syms && syms.length > 1) {
      this.base = syms.map(s => Buffer.isBuffer(s) ? s : Buffer.from(s, this._enc))
      this.digits = this._setDigits()
      this._rebuildLookups()
    }
  }
  ...
  setAlphabet(alpha) {
    if (alpha && alpha.length > 1) {
      alpha = Buffer.isBuffer(alpha) ? alpha.toString(this._enc) : alpha
      this.alphabet = Array.isArray(alpha) ? alpha.map(s => Buffer.isBuffer(s) ? s : Buffer.from(s, this._enc)) : this._graphemesToBuffers(alpha)
      this.digits = this._setDigits()
      this._rebuildLookups()
    }
  }
      ...
      try {
        ...
        for (let i = 0; i < this.digits; i++) {
          ...
          for (const { buff } of this._sortedBase) {
            if (e.slice(p + symLen, p + symLen + buff.length).equals(buff)) {
              symLen += buff.length
              on = true
              break
            }
          }
          if (!on) throw new Error(`Cannot find symbol ${i + 1} at position ${p + symLen}`)
        }
        p += symLen
